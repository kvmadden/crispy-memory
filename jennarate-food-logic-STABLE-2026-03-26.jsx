...
export default function App(){
  ...
  useEffect(function(){var t=gs2.theme||"auto";var isDark=t==="dark"||(t==="auto"&&window.matchMedia&&!window.matchMedia("(prefers-color-scheme:light)").matches);document.body.style.background=isDark?"#0D1117":"#FBF7F4";},[gs2.theme]);
  ...
  return(
    ...
    {/* HOME */}
      {vw==="home"&&<div className="fade" style={{display:"flex",flexDirection:"column",height:"100vh"}}>
        ...
        {/* ── Main content ── */}
        <div style={{flex:1,padding:"8px 16px 4px",display:"flex",flexDirection:"column",gap:8,overflow:"auto"}}>
...
function MoodQuiz(props){
  ...
  return <div className="fade">
    ...
    <div style={{padding:"20px 16px",display:"flex",flexDirection:"column",minHeight:"calc(100vh - 60px)"}}>
      ...
      {(function(){var allPending=pend[0]&&pend[1]&&pend[2];var anyUnanswered=qs.some(function(q,i){var key="_"+(phase==="narrow"?"n":"r")+round+"q"+i;return!ans[key];});if(!allPending||!anyUnanswered)return null;return <div style={{marginTop:16}}>
        <button className="jfl-cta" style={{padding:14,fontSize:16,fontWeight:700,width:"100%"}} onClick={function(){
          ...
          if(totalAnswered>=3){
            var best=null,bestScore=0,second=0;
            Object.keys(newAns).forEach(function(k){if(k.charAt(0)==="_")return;if(newAns[k]>bestScore){second=bestScore;bestScore=newAns[k];best=k;}else if(newAns[k]>second)second=newAns[k];});
            var gap=bestScore-second;
            if((gap>=4&&totalAnswered>=3)||(gap>=2&&totalAnswered>=6)||(totalAnswered>=9)){
              var moodId=QUIZ_MAP[best]||"balanced";
              var mood=MOODS.find(function(m){return m.id===moodId;})||MOODS.find(function(m){return m.id==="balanced";});
...
var CSS = [
  ...
  ".theme-light{--bg0:#FBF7F4;--bg1:#FFFFFF;--bg2:#F8F2ED;--bdr:#E5D5CA;--tx1:#1A1210;--tx2:#5C4A42;--tx3:#9A857A;--ac:#D6246B;--grn:#A07828;--yel:#B8860B;--red:#C0392B}",
  ...
  ".jfl-root{font-family:'IBM Plex Sans',system-ui,sans-serif;background:var(--bg0);min-height:100vh;max-width:480px;margin:0 auto;color:var(--tx1);-webkit-font-smoothing:antialiased;text-wrap:pretty}",

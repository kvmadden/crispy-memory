...
var _selWithCtx=useCallback(function(s){return Object.assign({},s,{_sessionSkips:sessionSkips});},[sessionSkips]);
...
var fpData=useMemo(function(){var fpSel={sp:["jenna"],mood:"safe-default",budget:"normal",speed:"normal",fam:"familiar",ar:false,kf:false,go:false,lo:false};var sc=scoreAll(_filterSkips(rests),fpSel,ppl,hist,mctx,gs2);if(!sc||sc.length<1)return null;var fp=sc[0];if(fp.score<55)return null;var gap=sc.length>1?(fp.score-sc[1].score):20;var prob=gap>15?"High likelihood":gap>8?"Probable choice":"Possible choice";return{fp:fp,prob:prob};},[rests,ppl,hist,mctx,gs2,_filterSkips]);
...
{/* HOME */}
  {vw==="dashboard"&&<div className="fade" style={{display:"flex",flexDirection:"column",height:"100dvh"}}>
    ...
    <div style={{flex:1,padding:"6px 16px 0",display:"flex",flexDirection:"column",gap:6,overflow:"auto"}}>
      ...
      <button className="jfl-cta" style={{flex:1,padding:"10px 8px",borderRadius:14,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2}} onClick={function(){setSel(function(s){return Object.assign({},s,{sp:["kevin","jenna"],mood:"roulette",budget:"normal",speed:"normal",fam:"surprise",kf:false,go:false,xa:0,xk:0});});setBusy(true);setTimeout(function(){var surpriseSel={sp:["kevin","jenna"],mood:"roulette",budget:"normal",speed:"normal",fam:"surprise",kf:false,go:false,xa:0,xk:0,_sessionSkips:sessionSkips};setRes(top3(scoreAll(_filterSkips(rests),surpriseSel,allPpl,hist,mctx,gs2)));setRrc(0);setResIdx(0);setBusy(false);go("results");},RESOLVE_DELAY);}} aria-label="Surprise me" title="Surprise me"><span style={{fontSize:22}}>{"\uD83C\uDFB2"}</span><span style={{fontSize:11,fontWeight:700,opacity:.9}}>Surprise</span></button>
      </div>
  ...
  {vw==="step1"&&<div className="fade" style={{display:"flex",flexDirection:"column",height:"100dvh"}}>
    ...
    <div style={{flex:1,padding:"16px",display:"flex",flexDirection:"column",overflow:"auto"}}>
      ...
      <button onClick={function(){setSel(function(s){return Object.assign({},s,{_pickIndividual:!s._pickIndividual,_showAllPeople:false});});}} className="jfl-label" style={{marginTop:14,marginBottom:8,background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",padding:0,display:"flex",alignItems:"center",gap:4}}><span>Or pick individually</span><span style={{fontSize:10,color:"var(--tx3)",transition:"transform .2s",transform:sel._pickIndividual?"rotate(90deg)":"rotate(0deg)"}}>{"\u203A"}</span></button>
      {sel._pickIndividual&&<div style={{display:"flex",flexWrap:"wrap",gap:6}}>
        ...
        {sel._showAllPeople&&<button onClick={function(){setSel(function(s){return Object.assign({},s,{_showAllPeople:false});});}} style={{padding:"6px 12px",borderRadius:20,border:"1px dashed var(--bdr)",background:"none",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:500,color:"var(--tx3)"}}>{"\u2039 fewer"}</button>}
      </div>}
